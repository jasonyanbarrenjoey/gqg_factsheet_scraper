...
factsheets_dir = Path("/workspaces/gqg_factsheet_scraper/factsheets")
outputs_dir = Path("/workspaces/gqg_factsheet_scraper/outputs")

outputs_dir.mkdir(exist_ok=True)



for json_file in factsheets_dir.rglob("*.json"):
    outputs_fund_dir = Path(f"/workspaces/gqg_factsheet_scraper/outputs/{json_file.stem}")

    outputs_fund_dir.mkdir(exist_ok=True)

    with open(json_file, "r") as f:
        fund_data = json.load(f)
    
        # Process fund_data here
        ...
        gics_ranked_excel = outputs_fund_dir / f"{base_name}_gics_ranked_format.xlsx"
        holdings_ranked_excel = outputs_fund_dir / f"{base_name}_holdings_ranked_format.xlsx"
        countries_ranked_excel = outputs_fund_dir / f"{base_name}_countries_ranked_format.xlsx"
            ...
            def parse_countries_table(countries_text, month):
                ...
                for line in data_lines:
                    ...
                    if match:
                        ...
                        # Try to match 3 columns (Country, Fund, Difference) - Index missing
                        match2 = re.match(r"^([A-Za-z\s]+)\s+([\d\.\-]+)\s+([\d\.\-]+)", line.strip())
                        if match2:
                            country, fund, diff = match2.groups()
                            records.append({
                                "Month": month,
                                "Entity": country.strip(),
                                "Fund": safe_float(fund),
